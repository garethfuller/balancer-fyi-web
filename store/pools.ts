...
import poolsQuery, { PoolArgs } from '~/lib/subgraph/queries/pools'
...
export const actions: ActionTree<PoolsState, RootState> = {
  async getAll ({ commit }: ActionContext<PoolsState, RootState>, args: PoolArgs) : Promise<Pool[]> {
    const poolsQueryJson = poolsQuery(args)
    const query = jsonToGraphQLQuery(poolsQueryJson)
  ...
  async getMore ({ commit }: ActionContext<PoolsState, RootState>, args: PoolArgs) : Promise<Pool[]> {
    const poolsQueryJson = poolsQuery(args)
    const query = jsonToGraphQLQuery(poolsQueryJson)
    const { data } = await this.$axios.$post(this.$ethConfig.subgraphUrl, { query })
    const pools = data.pools.map((pool: Pool) => serializePool(pool, this.$ethConfig))
